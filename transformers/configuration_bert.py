...
class BertMultipleLabelConfig(BertConfig):
    r"""
    A variant of Bert which introduce the Yago ontology info to the input.
    ...
    def __init__(self,
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2
            ...
            for key, value in json_config.items():
                self.__dict__[key] = value
            if "reference_size" not in self.__dict__:
                self.reference_size = reference_size
            if "sequence_length" not in self.__dict__:
                self.sequence_length = sequence_length
            if "lstm_hidden_size" not in self.__dict__:
                self.lstm_hidden_size = lstm_hidden_size
            if "lstm_dropout_prob" not in self.__dict__:
                self.lstm_dropout_prob = lstm_dropout_prob
            if "num_tags" not in self.__dict__:
                self.num_tags = num_tags
            if "use_crf" not in self.__dict__:
