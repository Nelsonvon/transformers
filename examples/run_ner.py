...
from qihui.data_processing.decode_preprocessing import decode_preprocessing
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    logger.info("***** {} results %s *****".format("Test" if mode=='test' else 'Evaluation'), prefix)
...
def main():
    ...
    parser.add_argument("--do_decode", action="store_true", help="Whether to run decoding.")
    ...
    parser.add_argument("--save_epoch", action="store_true", help="Save checkpoint every epoch.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Test all checkpoints starting with the same prefix as model_name ending and ending with step number",
    )
    ...
    if args.labels == "" and not args.do_decode:
    ...
    # name the output diretory according to the used model, time tag
    ...
    model.to(args.device)
    logger.info("Number of learnable parameters: {}".format(str(sum(p.numel() for p in model.parameters() if p.requires_grad))))
    
    if args.do_decode:
        assert(not (args.do_train or args.do_eval or args.do_predict))
        processed_file = os.path.join(args.data_dir, "decode.txt")
        sentence_length = args.max_seq_length-2
        decode_preprocessing(args.decode_file, processed_file, tokenizer, sentence_length, 'sent_line')
        starttime=datetime.datetime.now()
        result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="decode")
        output_test_predictions_file = os.path.join(args.output_dir, "decode_predictions.txt")
        with open(output_test_predictions_file, "w") as writer:
            with open(os.path.join(args.data_dir, "decode.txt"), "r") as f:
                example_id = 0
                for line in f:
                    if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                        writer.write(line)
