...
import os
from io import open
from qihui.model.utils_bert import ReferenceFeatures
import pickle
...
def convert_examples_to_features(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if yago_reference:
            reference_ids = [(list(ref_dict[input_id].keys()) if (input_id in ref_dict) else [tokenizer.pad_token_id]) for
                             input_id in input_ids]
            reference_weights = [(list(ref_dict[input_id].values()) if (input_id in ref_dict) else [0.0]) for input_id
                                 in input_ids]
            assert (len(reference_ids) == len(input_ids))

            for i in range(len(reference_ids)):
                assert (len(reference_ids[i]) == len(reference_weights[i]))
                reference_ids[i] += [tokenizer.pad_token_id]*(max_ref-len(reference_ids[i]))
        ...
        if pad_on_left:
            ...
            # print("pad_on_left")
            if yago_reference:
                reference_ids = ([[tokenizer.pad_token_id] * max_ref] * padding_length) + reference_ids
                reference_weights = [[0.0] * max_ref] * padding_length + reference_weights
        else:
            ...
            if yago_reference:
                reference_ids += [[tokenizer.pad_token_id] * max_ref] * padding_length
        ...
        if ex_index < 5 and print_samples:
            logger.info("*** Example ***")
        ...
        if yago_reference:
            ref_features.append(
                ReferenceFeatures(reference_ids=reference_ids,
