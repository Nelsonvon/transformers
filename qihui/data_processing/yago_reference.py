...
reference_type = 'th_prune' # th_prune / num_prune /  unlimit 
...
if True: #__name__= '__main__'
    ...
    with open('/work/smt3/wwang/TAC2019/qihui_data/yago/YagoReference{}_unlimit.pickle'.format('' if do_lower_case else '_cased'), 'rb') as fin:
        reference_dict = pickle.load(fin)
    with open('/work/smt3/wwang/TAC2019/qihui_data/yago/type_idx_dicts{}_unlimit.pickle'.format('' if do_lower_case else '_cased'), 'rb') as fin:
        type_idx_dict = pickle.load(fin)
    with open('/work/smt3/wwang/TAC2019/qihui_data/yago/idx_type_dicts{}_unlimit.pickle'.format('' if do_lower_case else '_cased'), 'rb') as fin:
    ...
    for token_id in reference_dict:
        ...
        assert(abs(sum(list(norm_reference.values()))-1)<1e-6)
        if reference_type == 'th_prune':
            prune_id = -1
            for ref_id, value in enumerate(list(norm_reference.values())):
                if value*1.0/sum_occur < PRUNE_TH:
                    prune_id = ref_id
                    break
            if prune_id >= 0:
                norm_reference = {k: v for k, v in list(norm_reference.items())[:prune_id]}
        sum_occur = sum(list(norm_reference.values()))
    ...
    # YagoReference_prune: remove probability which is too small < 1e-4
    ...
    print("Max number of types: {}".format(str(max(list([len(norm_ref) for norm_ref in list(reference_dict.values())])))))
    print("Avg number of types: {}".format(str(sum(list([len(norm_ref) for norm_ref in list(reference_dict.values())]))/len(reference_dict))))
