...
# import os, sys
# sys.path.append("/u/qfeng/Project/tac19/")
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # switch the order of optimizer step and scheduler step according to the warning information in the log
                optimizer.step()
    ...
    if args.do_significant_check and args.yago_reference:
        ...
        for id in range(config.vocab_size):
            if id in ref_dict:
                ...
                reference_weights = model.bert.embeddings.ref_weights[id].to(device)
                # logger.info(reference_ids.size())
                # logger.info(reference_weights.size())
                reference_embedding = torch.sum(
                    model.bert.reference_embeddings(reference_ids) * torch.unsqueeze(reference_weights, dim=-1), dim=-2)
